...
class Node:
    ...
    def __init__(self, G=0, H=0, coordinate=0, parent=0):
        self.G = G
        self.H = H
        self.F = G + H
        self.parent = parent
...
def boundary_and_obstacles(start, goal, top_vertex, bottom_vertex, obs_number):
    ...
    bx = list(range(bottom_vertex[0] + 6, top_vertex[0]))
    by = [bottom_vertex[1]] * len(bx)
    dx = [bottom_vertex[0]] + bx + [top_vertex[0]]
    dy = [top_vertex[1]] * len(dx)

    # generate random obstacles
    ob_x = np.random.randint(bottom_vertex[0] + 2,
                             top_vertex[0], obs_number).tolist()
    ob_y = np.random.randint(bottom_vertex[1] + 2,
...
def draw(close_origin, close_goal, start, end, bound):
    ...
    plt.plot(end[0], end[1], '*b', label='Vehicle start point')
    plt.plot(start[0], start[1], '^b', label='Vehicle stop point')
    plt.legend()
    plt.pause(0.000001)
    ...
    if flag == 0:
        ...
        if node_intersect:  # a path is find
            ...
            if show_animation:  # draw the path
                ...
                plt.title('vehicle has arrived to its destination', size=20, loc='center')
                plt.pause(0.01)
                plt.show()
...
def main(obstacle_number=1000):
    print(__file__ + ' start!')

    top_vertex = [500, 500]  # top right vertex of boundary
    bottom_vertex = [50, 50]  # bottom left vertex of boundary
